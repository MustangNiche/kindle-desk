...
LOG=/mnt/us/dashboard/upside-down.log
[ -d "$BASE" ] || exit 1
exec >>"$LOG" 2>&1
printf '\n=== UPSIDE_DOWN_REVISION=2 ===\n'
date
...
. /mnt/us/dashboard/screen-rotation.sh || exit 1
restore_ui() {
...
"$FBINK" -q -f -c -g "file=/mnt/us/dashboard/direct/base.png,w=-1,h=-1" || exit 1
draw_day "$day"
minute=$(clock_key)
...
last_battery=
update_battery
. /mnt/us/dashboard/direct-runtime.sh || exit 1
lua /mnt/us/dashboard/gre-update.lua "$FBINK" || exit 1
lua /mnt/us/dashboard/weather-update.lua "$FBINK" || printf "WEATHER_UPDATE_ERROR=1\n"
